[Java Implementation Repo](https://github.com/brandon-mason/OpenAI-Image-Gen-Java)

Local Run Command: npm run dev

To use this app with your own key, create a .env file with the following variables: <br />
- VITE_KEY='{your OpenAI API key}' <br />
OPTIONAL:
- VITE_PW='{your password}' <br />

Include 'VITE_PW' if you would like to password protect your application.

<p align="center">
	<img src="https://github.com/brandon-mason/OpenAI-Image-Gen-JS/blob/main/readme-assets/Loading.gif" width="75%">
</p>

<p align="center">
	<img src="https://github.com/brandon-mason/OpenAI-Image-Gen-JS/blob/main/readme-assets/ImageCache.gif" width="75%">
</p>
